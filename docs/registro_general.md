...
# Implementación de Funcionalidad Multi-Empresa en RH-Plus
**Fecha:** 27 de mayo de 2025

## Descripción

Este commit implementa la funcionalidad de multi-empresa en el sistema RH-Plus, permitiendo que múltiples empresas utilicen la plataforma con sus propios usuarios y datos aislados. La implementación incluye un nuevo sistema de registro que contempla diferentes tipos de registro (usuario individual, creación de nueva empresa, o unirse a empresa existente), así como la gestión de roles y permisos a nivel de empresa.
...
1. **Modelo Company** 
   - Almacena información de empresas en el sistema
   - Campos principales: nombre, identificación fiscal, dirección, teléfono, email, sitio web
   - Estado de la empresa: pendiente, aprobado, rechazado
   - Campos de auditoría: fecha de creación, aprobación, y quién aprobó

2. **Modelo CompanyUser**
   - Implementa la relación muchos a muchos entre usuarios y empresas
   - Permite asignar roles específicos a usuarios dentro de cada empresa
   - Estados: pendiente, aprobado, rechazado
   - Facilita el cambio entre empresas para usuarios que pertenezcan a múltiples

3. **Modelo Role (Actualizado)**
   - Roles predefinidos: SUPERADMIN, COMPANYADMIN, HRMANAGER, SUPERVISOR, EMPLOYEE
   - Cada rol tiene permisos específicos en el contexto de una empresa

### Serializers

1. **UserRegistrationSerializer (Actualizado)**
   - Nuevos campos para manejo de empresas:
     - `register_type`: Tipo de registro ('new_company', 'join_company', 'no_company')
     - Campos para nueva empresa: nombre, identificación fiscal, dirección, etc.
     - Campo para unirse a empresa existente: ID de empresa
   - Validación personalizada según el tipo de registro
   - Lógica para creación de empresa y asignación de roles iniciales

2. **CompanySerializer y CompanyUserSerializer**
   - Serializan los modelos Company y CompanyUser para APIs
   - Validación de datos de empresa

### Vistas y APIs

1. **UserRegistrationView (Actualizado)**
   - Maneja los tres tipos de registro
   - Crea estructuras de datos según el tipo
   - Envía notificaciones a administradores

2. **CompanyViewSet y CompanyUserViewSet**
   - APIs para gestión de empresas y relaciones empresa-usuario
   - Endpoints para aprobación de empresas y usuarios

### Middleware

1. **CompanyContextMiddleware**
   - Filtra datos por empresa actual del usuario
   - Asegura que los usuarios solo vean información de su empresa

### Actualización de Modelos Existentes

Todos los modelos en los módulos siguientes fueron actualizados para incluir una relación con Company:
- Selection
- Affiliation
- Payroll
- Performance
...
1. **Company**
   ...
   - Contiene información básica y estado

2. **CompanyUser**
   - Representa la relación entre un usuario y una empresa
   - Incluye roles y estado de la relación

### Actualización de Autenticación

1. **AuthProvider**
   - Método `register` actualizado para soportar tres tipos de registro:
     - Usuario individual (`no_company`)
     - Nueva empresa (`new_company`)
     - Unirse a empresa existente (`join_company`)
   - Manejo de campos adicionales según el tipo de registro

### Interfaz de Usuario

1. **RegisterScreen**
   - Interfaz actualizada para incluir selección de tipo de registro
   - Campos condicionales que aparecen según el tipo seleccionado:
     - Para nueva empresa: nombre, identificación fiscal, dirección, etc.
     - Para unirse a empresa: selector de empresa existente
   - Validación de formulario según el tipo de registro
   - Manejo de errores específicos por campo

## Flujo de Trabajo Implementado

1. **Registro de Nueva Empresa**
   - Usuario se registra como representante de empresa
   - Completa información de la empresa
   - Sistema marca la empresa como "Pendiente"
   - SuperAdmin recibe notificación y puede aprobar/rechazar
   - Al aprobar, usuario se convierte en CompanyAdmin

2. **Registro para Unirse a Empresa**
   - Usuario selecciona empresa existente
   - Sistema marca al usuario como "Pendiente"
   - CompanyAdmin recibe notificación
   - CompanyAdmin aprueba/rechaza la solicitud

3. **Usuario Individual**
   - Registro sin asociación a empresa
   - Puede solicitar unirse a empresa posteriormente

## Consideraciones de Seguridad

1. **Aislamiento de Datos**
   - Toda consulta filtra por empresa del usuario
   - Usuarios solo ven información de su empresa

2. **Control de Acceso**
   - Roles específicos por empresa
   - Aprobación necesaria para acceder a empresas

3. **Auditoría**
   - Registro de todas las acciones de aprobación
   - Trazabilidad de cambios en empresas y usuarios

## Próximos Pasos

1. **Mejora del Selector de Empresas**
   - Implementar búsqueda de empresas en lugar de solo ID

2. **Panel de Aprobaciones**
   - Interfaz dedicada para gestionar solicitudes

3. **Sistema de Notificaciones**
   - Notificaciones en tiempo real para aprobaciones

4. **Cambio de Empresa Activa**
   - Permitir a usuarios con múltiples empresas cambiar entre ellas

5. **Gestión de Roles Avanzada**
   - Interfaz para asignar/revocar roles dentro de una empresa

## Notas Técnicas

- La implementación requirió migraciones significativas en la base de datos
- Se utilizó un enfoque modular para mantener el código organizado
- Se siguieron los principios de aislamiento de datos para garantizar la seguridad
- Se mantuvieron las funcionalidades existentes intactas mientras se agregaban las nuevas

Este commit establece la base para un sistema multi-empresa completo, con un enfoque en la seguridad, escalabilidad y usabilidad.